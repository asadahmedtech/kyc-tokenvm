...
var _ = ginkgo.BeforeSuite(func() {
	...
	for _, inst := range instances {
		...
		for _, alloc := range g.CustomAllocation {
			balance, _, err := cli.Balance(context.Background(), alloc.Address, ids.Empty)
			gomega.Ω(err).Should(gomega.BeNil())
...
var _ = ginkgo.Describe("[Tx Processing]", func() {
	...
	ginkgo.It("Gossip TransferTx to a different node", func() {
		...
		ginkgo.By("ensure balance is updated", func() {
			balance, _, err := instances[1].tcli.Balance(context.Background(), sender, ids.Empty)
			gomega.Ω(err).To(gomega.BeNil())
			gomega.Ω(balance).To(gomega.Equal(uint64(9899717)))
			balance2, _, err := instances[1].tcli.Balance(context.Background(), sender2, ids.Empty)
			gomega.Ω(err).To(gomega.BeNil())
	...
	ginkgo.It("processes valid index transactions (w/block listening)", func() {
		...
		// Fetch balances
		balance, _, err := instances[0].tcli.Balance(context.TODO(), sender, ids.Empty)
		gomega.Ω(err).Should(gomega.BeNil())
		...
		balancea, _, err := instances[0].tcli.Balance(context.TODO(), sender, ids.Empty)
		gomega.Ω(err).Should(gomega.BeNil())
	...
	ginkgo.It("create a new asset (simple metadata)", func() {
		...
		asset1ID = tx.ID()
		balance, _, err := instances[0].tcli.Balance(context.TODO(), sender, asset1ID)
		gomega.Ω(err).Should(gomega.BeNil())
	...
	ginkgo.It("mint a new asset", func() {
		...
		balance, _, err := instances[0].tcli.Balance(context.TODO(), sender2, asset1ID)
		gomega.Ω(err).Should(gomega.BeNil())
	...
	ginkgo.It("mints another new asset (to self)", func() {
		...
		balance, _, err := instances[0].tcli.Balance(context.TODO(), sender, asset2ID)
		gomega.Ω(err).Should(gomega.BeNil())
		gomega.Ω(balance).Should(gomega.Equal(uint64(10)))
	})
	...
	ginkgo.It("mints another new asset (to self) on another account", func() {
		...
		balance, _, err := instances[0].tcli.Balance(context.TODO(), sender2, asset3ID)
		gomega.Ω(err).Should(gomega.BeNil())
		gomega.Ω(balance).Should(gomega.Equal(uint64(10)))
	})
	...
	ginkgo.It("fill order with sufficient balance", func() {
		...
		balance, _, err := instances[0].tcli.Balance(context.TODO(), sender, asset3ID)
		gomega.Ω(err).Should(gomega.BeNil())
	...
	ginkgo.It("close order", func() {
		...
		balance, _, err = instances[0].tcli.Balance(context.TODO(), sender2, asset2ID)
		gomega.Ω(err).Should(gomega.BeNil())
