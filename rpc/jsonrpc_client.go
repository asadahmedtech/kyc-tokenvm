...
func (cli *JSONRPCClient) Balance(ctx context.Context, addr string, asset ids.ID) (uint64, storage.KYCData, error) {
	...
	return resp.Amount, resp.KYCData, err
}

func (cli *JSONRPCClient) KYC(ctx context.Context, addr string) (uint8, uint8, []byte, error) {
	resp := new(KYCReply)
	err := cli.requester.SendRequest(
		ctx,
		"kyc",
	...
	return resp.KYCCountry, resp.KYCAuthority, resp.KYCMetadata, err
}
...
func (cli *JSONRPCClient) WaitForBalance(
	...
	return rpc.Wait(ctx, func(ctx context.Context) (bool, error) {
		balance, _, err := cli.Balance(ctx, addr, asset)
		if err != nil {
			return false, err
		}
