...
type BalanceReply struct {
	...
	KYCData storage.KYCData `json:"kycdata"`
}
...
func (j *JSONRPCServer) Balance(req *http.Request, args *BalanceArgs, reply *BalanceReply) error {
	...
	kycd, err := j.c.GetKYCFromState(ctx, addr)
	if err != nil {
		return err
	}
...
type KYCReply struct {
	KYCCountry   uint8  `json:"kyccountry"`
	KYCAuthority uint8  `json:"keyauthority"`
	KYCMetadata  []byte `json:"kycmetadata"`
}

func (j *JSONRPCServer) KYC(req *http.Request, args *KYCArgs, reply *KYCReply) error {
	ctx, span := j.c.Tracer().Start(req.Context(), "Server.KYC")
	defer span.End()
	...
	reply.KYCAuthority = kycd.KYCAuthority
	reply.KYCCountry = kycd.KYCCountry
	reply.KYCMetadata = kycd.KYCMetadata
