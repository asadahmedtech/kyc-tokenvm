...
func handleTx(c *trpc.JSONRPCClient, tx *chain.Transaction, result *chain.Result) {
	// fmt.Println(result.WarpMessage.Payload)
	...
	if result.Success {
		...
		case *actions.CreateKYC:
			var wt *actions.WarpTransfer
			var err error
			var out string
			if result.WarpMessage != nil && result.WarpMessage.Payload != nil {
				wt, err = actions.UnmarshalWarpTransfer(result.WarpMessage.Bytes())
			} else {
				out = string(result.Output)
			}
			summaryStr += fmt.Sprintf("destination: %s | %v | %s", wt, err, out)
			fmt.Println("lol")
		}
	}
	utils.Outf(
