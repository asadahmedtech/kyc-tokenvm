...
func lookupSetKeyBalance(choice int, address string, uri string, networkID uint32, chainID ids.ID) error {
	...
	balance, kyc, err := cli.Balance(context.TODO(), address, ids.Empty)
	if err != nil {
		return err
	}
	utils.Outf(
		"%d) {{cyan}}address:{{/}} %s {{cyan}}balance:{{/}} %s %s {{cyan}}kyc:{{/}} %v\n",
		choice,
		address,
...
func lookupKeyKYC(pk ed25519.PublicKey, uri string, networkID uint32, chainID ids.ID, assetID ids.ID) error {
	_, _, _, _, err := handler.GetAssetInfo(
		context.TODO(), trpc.NewJSONRPCClient(uri, networkID, chainID),
		pk, assetID, true)
	return err
